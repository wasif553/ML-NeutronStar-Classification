...
#cd(string(homedir(),"\\Julia Codes\\Neutron Star"))
cd(string("c:/Users/Elitebook/Documents/PhD/Theo code"))
...
#Overlap
const λ3=0.02*fm^-1 #0.02-0.03. note that 0.02 is the preferred term
const E0=5500*MeV
const b=0.50*fm
...
function overlap(nn,np,nΛ,nΞ0,nΞm)
    ...
    return overlap=E0*exp(-(rbar^2)/b^2)*(nn+np+nΛ+nΞ0+nΞm)
end
...
nrange=collect(0.0:dens_step:1.2fmm3) #beta equilibrium
...
@time fitresult=NMfitδ(3fm^2,[0.16fmm3,-15.8MeV,30MeV])
...
nparameters2=NMparametersFull(fitresult...)./(fmm3,MeV,MeV,MeV,MeV)
...
#Btab1=readdlm("E5500 hyperons Species Btab.dat")
#Btab2=readdlm("delta no overlap Species Btab.dat")
#Species Fraction
plot()
...
plot!(xaxis=[0,1],yaxis=[1e-3,1e0],xlabel="n\$_b\$(fm\$^{-3}\$)",ylabel="Species Fraction, log(n\$_i\$/n\$_b\$)",guidefontsize=8,title_location=:left,xticks=0:0.1:2,frame=:box,size=(600,300))
plot!(legendfontsize=6,legend=:outerright)
...
############Parameters play ############
plot(Etab2./mevpfm3, Ptab2./mevpfm3, linestyle=:dot, color=:red, label="No Overlap (nucleons)")
plot!(xlabel="Energy(MeV.fm\$^{-3}\$)", ylabel="Pressure(MeV.fm\$^{-3}\$)")
plot!(xaxis=[0,1000],yaxis=[0,200])
plot!(legend=:topleft)
...
#
function ∂y(r,Yr,Pr,Mr,ϵ,del=0.001*mevpfm3*fmm4_new)
    y1=Yr^2
...
function oneTOV(ϵ,nVe,P0,Rmax)
    ...
    while r<Rmax
        ...
        m=∂M(r,p,m,ϵ)*δ + m
        y=∂y(r,y,p,m,ϵ)*δ + y
        Aa=δ*4*π*r^2*nVe(ϵ(p))*(1-2*G*m/r)^(-1/2) + Aa
    ...
    return resP,resM./Msol,resR,r,resY,resA
...
function MRall(ϵ,nVe,P0)
    ...
    for k in -2:0.02:1.4
        ...
        p1tov,m1tov,x,r1tov,y1tov,totalA=oneTOV(ϵ,nVe,P0*10^k,100km)
        push!(M,m1tov[end])
        push!(R,r1tov)
        push!(A,totalA[end])
        push!(Y,y1tov[end])
...
#mtest1,rtest1,ytest2=MRall(Eeos1,Ptab2[100].*fmm4_new)
mtest2,rtest2,ytest2,totalbaryon=MRall(Eeos2,nVe2,Ptab2[100].*fmm4_new)

#########################

plot([0,23], [1.908,1.908],color=:gray,lw=3,label="PSR-J1714")
    plot!(rtest2,mtest2,color=:dodgerblue, label="B=0.50fm")
...
#savefig("TOV with and without delta.pdf")
...
plot(mtest2,love2 .* (mtest2./rtest2 .*Msol).^-5 .* (2/3), xaxis=([1,1.7],1:0.1:2),yaxis=[0,3000], label="Overlap")
plot!([1.4, 1.4], [70,580], color=:black, label="Observation")
plot!(xlabel="M[M⊙]",ylabel="Λ")
writedlm("pressure.dat", Ptab2)
